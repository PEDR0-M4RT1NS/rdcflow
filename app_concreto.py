...
# Chama a proteção de tela e login
mostrar_sidebar()
...
with st.form("form_final", clear_on_submit=True):
    ...
    with c1:
        # Data como texto para preenchimento rápido
...
if submit:
    try:
        # Lógica de Tempo e Valor (Convertendo os textos formatados em objetos de tempo)
        fmt = "%H:%M"
        ...
        st.error("Preencha a Data (8 dígitos) e os Horários (4 dígitos) corretamente!")
...
if os.path.isfile('historico_concretagem.csv'):
    ...
    # KPIs e Gráficos continuam aqui como você já tinha...
    data_para_filtrar = st.date_input("Filtrar Resumo por Data:", datetime.now())
    data_formatada = data_para_filtrar.strftime("%d/%m/%Y")
    df_dia = df[df['DATA'] == data_formatada]
    
    k1, k2, k3 = st.columns(3)
    k1.metric("Volume Total", f"{df_dia['m3'].sum():.2f} m³")
    k2.metric("Gasto Total", f"R$ {df_dia['VALOR_TOTAL'].sum():,.2f}")
