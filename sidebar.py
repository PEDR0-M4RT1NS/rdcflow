...
def carregar_usuarios():
    try:
        # Tenta ler o CSV. Se não tiver a coluna 'nivel', ele cria uma padrão.
        df = pd.read_csv('usuarios.csv')
        if 'nivel' not in df.columns:
            df['nivel'] = 'USER' # Valor padrão caso a coluna não exista ainda
            df.to_csv('usuarios.csv', index=False)
        return df
    except:
        # Se o arquivo não existir, cria o ADM mestre
        ...
        with col2:
            ...
            if st.button("ENTRAR NO SISTEMA"):
                ...
                if not user_match.empty:
                    # SALVAMOS TUDO NA SESSÃO PARA USAR DEPOIS
